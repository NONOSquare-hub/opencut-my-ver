...
export class VideoCache {
	...
	async getFrameAt({
		...
		const current = previous.then(async () => {
			...
			try {
				...
				console.warn(
					`VideoCache resolveFrame error for ${mediaId} (e.g. codec reclaimed), re-initializing:`,
					error,
				);
				this.clearVideo({ mediaId });
				await this.ensureSink({ mediaId, file });
				const newSinkData = this.sinks.get(mediaId);
				if (!newSinkData) return null;
				return await this.seekToTime({ sinkData: newSinkData, time });
			}
		});
		this.frameChain.set(
			mediaId,
	...
	private async initializeSink({
		...
		if (this.sinks.size >= 6) {
			const oldestKey = this.sinks.keys().next().value;
			if (oldestKey) {
				this.clearVideo({ mediaId: oldestKey });
			}
		}
