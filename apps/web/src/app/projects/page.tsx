...
import { StorageOverviewCard } from "@/project/components/storage-overview-card";
...
function ProjectItem({
	...
	useEffect(() => {
		...
		storageService
			...
			.then((info) => {
				if (isMounted) {
					setStorageSize(formatStorageBytes({ bytes: info.totalBytes }));
				}
			})
			.catch(() => {});
		return () => {
		...
		<Card className="bg-background overflow-hidden border-none p-0">
			...
			<CardContent className="flex flex-col gap-2 px-0 pt-4">
				...
				<div className="text-muted-foreground flex items-center justify-between gap-1.5 text-xs">
					...
					{storageSize && (
						<div className="flex items-center gap-1 font-mono text-[11px] text-muted-foreground bg-muted/60 px-1.5 py-0.5 rounded">
	...
	const listRowContent = (
		<div className="flex items-center gap-3 flex-1 min-w-0">
			...
			{storageSize && (
				<span className="text-muted-foreground text-xs font-mono shrink-0 hidden md:flex items-center gap-1 bg-muted/40 px-2 py-0.5 rounded">
