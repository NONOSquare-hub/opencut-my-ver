...
export class ProjectManager {
	...
	async export({ options }: { options: ExportOptions }): Promise<ExportResult> {
		...
		const startedAt = this.exportState.startedAt || Date.now();
		...
		const result = await this.editor.renderer.exportProject({
			...
			onProgress: ({ progress }) => {
				...
				const estimatedTotalMs = progress > 0 ? elapsedMs / progress : null;
				const estimatedRemainingMs = estimatedTotalMs
					? Math.max(estimatedTotalMs - elapsedMs, 0)
					: null;
				this.exportState = {
					...this.exportState,
					elapsedMs,
				...
				if (typeof window !== "undefined" && window.electronAPI?.setExportProgress) {
					window.electronAPI.setExportProgress(progress);
				}
			},
		...
		if (typeof window !== "undefined" && window.electronAPI?.setExportProgress) {
			window.electronAPI.setExportProgress(-1);
		}
